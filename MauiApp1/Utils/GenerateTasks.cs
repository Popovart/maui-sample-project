...
{
    public static List<TaskModel> Create(int count = 20, int? seed = null)
    {
        ...
        {
            var id = Guid.NewGuid();
            var name = $"Task #{i}: {SampleTitles[random.Next(SampleTitles.Length)]}";
            var description = random.NextDouble() < 0.7
                ? SampleDescriptions[random.Next(SampleDescriptions.Length)]
                : null;
